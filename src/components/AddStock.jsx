...
export default function Allot({isOpen, onClose, onSuccess, tx}) {
    ...
    useEffect(() => {
        if (tx) {
            ...
            setMediumBooks(tx.medium_books || 0);
        }
    }, [tx, isOpen]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm font-outfit">
            <div className="bg-white p-4 sm:p-8 rounded-xl shadow-2xl w-full max-w-xs sm:max-w-md relative animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} >
                    ...
                    <div className="mb-4 flex">
                        <label htmlFor="mahaBigBooks" className="block text-sm font-medium text-gray-700">Medium Books</label>
                        <input
                            type="number"
                            id="mediumBooks"
                            name="mediumBooks"
                            value={mediumBooks}
                            onChange={(e) => setMediumBooks(e.target.value)}
                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                        />
                    </div>
                    ...
                    <div className="mb-4 flex gap-2 items-center">
                        ...
                        <span id="quantity" name="quantity" className="" >{Number(smallBooks) + Number(bigBooks) + Number(mediumBooks)}</span>
                    </div>
